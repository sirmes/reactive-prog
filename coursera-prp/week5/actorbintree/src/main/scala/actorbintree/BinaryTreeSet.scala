...
class BinaryTreeSet extends Actor with Stash {
  ...
  def garbageCollecting(newRoot: ActorRef): Receive =  {
    case operation: Operation => stash()
    case CopyFinished => {
      root ! PoisonPill
      root = newRoot
      unstashAll()
...
class BinaryTreeNode(val elem: Int, initiallyRemoved: Boolean) extends Actor {
  ...
  val normal: Receive = {
    ...
    case operation: Operation if (operation.elem < elem && subtrees.isDefinedAt(Left)) => subtrees(Left) ! operation
    case operation: Operation if (operation.elem > elem && subtrees.isDefinedAt(Right)) => subtrees(Right) ! operation
